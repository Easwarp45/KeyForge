        ...
        <div className="auth-card rounded-xl overflow-hidden p-8 flex flex-col gap-6 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="floating-label-group">
              <input type="text" id="name" placeholder=" " required value={name} onChange={(e) => setName(e.target.value)}
            ...
            >
              {loading ? (
                <><span className="material-symbols-outlined animate-spin" style={{ fontSize: 20 }}>progress_activity</span> Creating account...</>
              ) : (
                'Create Account'
              )}
            </motion.button>
          ...
          <p className="text-center text-on-surface-variant text-sm">
            ...
            <button onClick={() => navigate('/sign-in')} className="text-primary font-bold hover:underline">Sign in</button>
          </p>
        </div>
      </motion.main>
    </div>
